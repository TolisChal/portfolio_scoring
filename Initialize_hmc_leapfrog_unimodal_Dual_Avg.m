function [eps_step, X0, prob_step_in] = Initialize_hmc_leapfrog_unimodal_Dual_Avg(A, b, X0, sigma, y0, a, N, delta)
...
% of the correlation matrices
    ...
    for i = 1:N
        ...
        %for jj = 1:W
            ...
            %Ti_nx = T0i;
            %v_nx = v;
            %grad_x_pl = a * (2*(sigma*X0) - q*y0);
            grad_x_pl = a * (2*sigma*(X0 - y0));
            grad_x_min = grad_x_pl;
            h1 = a * ((X0-y0)'*sigma*(X0-y0)) + 0.5 * (v' * v);
            %exp(-h1)
            uu = log(rand) - h1;
            %x_pl = [X0(lower); T0i];
            %x_min = [X0(lower); T0i];
            ...
            while(s == 1)
                ...
                for k = 1:2^j
                    ...
                    grad_x = a * (2*(sigma*(X-y0))); 
                    v = v - (eps_step/2) * grad_x;
  
                    hj = a * ((X-y0)'*sigma*(X-y0)) + 0.5 * (v' * v);
