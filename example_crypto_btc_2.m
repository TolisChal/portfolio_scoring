...
MM = 10;
samples_all = {};
[as_uni1, as_dense_uni, dists_dense_uni, samples_uni] = compute_a_sequence_unimodal(sigma, max_vol, btc_ptf, N, R, r);
%int_ratios_uni = compute_integral_ratios_fixed_q_all_opt_unimodal(sigma, btc_ptf, as_uni1, N, R, r, samples_uni);
[as_uni, dists_uni, indxs_uni] = cut_as_3(as_uni1, dists_dense_uni, MM);
btc_skiped = false;
ii=1;
while(iter <= (length(qs)+1))
    i = qs(ii)
    if (vols(iter) > btc_vol & ~btc_skiped)
        %if(vols(iter) > btc_vol)
            btc_skiped = true;
...
save('advanced_example_12_strategies_medium_risk_results_mean.mat','w','vols','dists_all','indxs_cuted','samples_all', '-v7.3')
